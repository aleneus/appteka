...
** DONE [e] Add docs
* 0.9.0
** DONE [t] ref: rename codetextedit.py to code_text_edit.py
** DONE [e] ref: brush names in code_text_edit.py
** DONE [t] ref: MultiWaveform.add_plot: deprecate main argument
** DONE [t] test: update for CodeTextEdit class
** DONE [e] docs: add release checklist
** DONE [t] test: put all multiwaveform tests to one class
** DONE [t] lint: up result
* later
** TODO [b] fix: pyqt.testing: multiwaveform: can not run several test classes in one file
** TODO [t] ref: setup_state(self, state_name, invisible=[], disabled=[])
** TODO [p] feat: pyqtgraph: new widget for waveforms
** TODO [t] ref: redloc pyqtgraph.waveform
** TODO [t] feat: pyqtgraph.waveform: Add 'synchronized' to state
** TODO [p] feat: make Plot class and add get_plot() to waveforms classes
** TODO [t] docs: add demo for test_sqlite_schema
** TODO [p] feat: configuration: tools for adding of unexisting parameters and sections
** TODO [p] feat: find something more convenient than conf.setting[key]
** TODO [t] depr: remove MultiWaveform.set_plot_color()
** TODO [t] depr: remove TestDialog.set_text()
** TODO [t] depr: remove codetextedit.py
